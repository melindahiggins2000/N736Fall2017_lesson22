...
m1.aov <- Anova(m1, idata = data.frame(tfactor),
                idesign = ~tfactor, type="III")
summary(m1.aov, multivariate=FALSE)
summary(m1.aov, multivariate=TRUE)


library(psych)
coeff <- psych::ICC(na.omit(h1))
print(coeff)

library(irr)
irr::icc(h1)

h1long <- tidyr::gather(h1, pcstime, pcsvalue, 
                        pcs:pcs4, factor_key=TRUE)
h1long <- h1long %>%
...
# complete pooling
m1 <- lm(pcsvalue ~ 1, data=h1long)
m1

# no-pooling include subject index
m2 <- lm(pcsvalue ~ 1 + factor(id)-1, data=h1long)
m2
hist(coef(m2))

library(lme4)
# use lmer - varying intercept no predictors
m0 <- lmer(pcsvalue ~ 1 + (1 | id), data=h1long)
m0
diplay(m0)
...
head(cm0$id)
summary(cm0$id)
head(cm0$id$`(Intercept)`)
hist(cm0$id$`(Intercept)`)
